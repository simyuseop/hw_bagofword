...
from student import build_vocabulary, get_bags_of_words, \
    svm_classify, sift_bocabulary, get_sift
...
def projSceneRecBoW():
    ...
    elif FEATURE.lower() == 'sift_like':
        if not os.path.isfile('vocab_sift.npy'):
            print('No existing visual word vocabulary found. Computing one from training images.')
            ...
            # YOU CODE build_vocabulary (see student.py)
            vocab = sift_bocabulary(train_image_paths, vocab_size)
            np.save('vocab_sift.npy', vocab)
        #Yu-seop making main code
        train_image_feats = get_sift(train_image_paths)
        test_image_feats = get_sift(test_image_paths)
